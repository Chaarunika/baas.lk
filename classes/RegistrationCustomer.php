    ...
    private function registerNewUser($user_name, $user_email, $user_password, $user_password_repeat)
    {
        ...
        // finally if all the above checks are ok
            ...
            if (count($result) > 0) {
				...
				$query_new_user_insert->bindValue(':user_catagory',$customer, PDO::PARAM_STR);
				$query_new_user_insert->bindValue(':user_avatar',$user_avatar, PDO::PARAM_STR);
                $query_new_user_insert->execute();

                // id of new user
                $user_id = $this->db_connection->lastInsertId();

                $query_new_user_insert = $this->db_connection->prepare('INSERT INTO customer(user_id) VALUES(:user_id)');
